...
void main()
{
    ...
    // Terrains data
    // Check if we have valid terrain (offsets < 999999 indicates terrain chunks exist)
    // Sentinel value 999999.0 is used to indicate flat terrain/no terrain data
    const float INVALID_OFFSET = 999999.0;
    bool hasValidTerrain = (abs(uTerrainAOffset.x) < INVALID_OFFSET && abs(uTerrainAOffset.y) < INVALID_OFFSET) ||
                           (abs(uTerrainBOffset.x) < INVALID_OFFSET && abs(uTerrainBOffset.y) < INVALID_OFFSET) ||
                           (abs(uTerrainCOffset.x) < INVALID_OFFSET && abs(uTerrainCOffset.y) < INVALID_OFFSET) ||
                           (abs(uTerrainDOffset.x) < INVALID_OFFSET && abs(uTerrainDOffset.y) < INVALID_OFFSET);
    
    vec4 terrainData = vec4(0.0, 0.0, 1.0, 0.0); // Default: no height, normal pointing up
